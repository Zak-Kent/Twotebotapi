...
from twotebotapp.secrets import CONSUMER_KEY, CONSUMER_SECRET, ACCESS_TOKEN, ACCESS_TOKEN_SECRET  # NOQA

loggly = logging.getLogger('loggly')


# tags related to pycon and tw_testy
DEFAULT_QUERIES = ['pycon2017', 'pycon 2017']

# 'pycon,pycon2017, "pycon 2017",pyconopenspaces'
...
from twotebotapp import models  # NOQA
...
class Bot(object):
    ...
    def send_tweet(self, message):
        ...
        # return message
        # will need to add the send action here when bot wants to send
        pass
    ...
    def get_tweets(self, ids):
        ids = [str(ids)] if isinstance(ids, (str, int)) else list(ids)
        tweets = []
        for i in range(int(len(ids) / 100.) + 1):
            try:
                ...
                # this gets hit after a few searches tw_testy, because bot is no longer global throws a bot not 
                # defined error
                if not i % 2:
    ...
    def process_queue(self, ids=None):
        ...
        # don't have save table needed to save tweets
...
def run_bot_search():
    ...
    # counts number of tweets already in db
        ...
        for qid, ht in enumerate(hashtags):
            ...
            try:
                for tweet in bot.search(ht, args['num_tweets']):
                    ...
                    if acceptable_tweet:
                        try:
                            # don't have save tweet table, just printing scraped tweet
                            # last_tweets += [bot.save_tweet(acceptable_tweet)]
                            print("*" * 80)
                            print("found this tweet: {}".format(acceptable_tweet))
                            print("*" * 80)
            ...
            # don't have matching Tweet table needed in this method
            # num_after = bot.count()
            ...
            # missing table needed for count feature
